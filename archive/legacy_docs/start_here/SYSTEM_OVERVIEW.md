# AI Sales OS v6.0 — System Overview

## Architecture

```
┌──────────────┐     ┌──────────────────────────┐     ┌──────────────┐     ┌───────────────┐
│  Apollo.io   │────►│  8 Python Modules (26)    │────►│   Notion     │────►│ GitHub Actions│
│  (Data)      │     │  Core + Scoring +         │     │  (CRM Hub)   │     │ (Daily Cron)  │
└──────────────┘     │  Automation + Governance +│     └──────────────┘     └───────────────┘
 45,086 contacts     │  Enrichment + Meetings +  │      7 Databases          2-job pipeline
 15,407 companies    │  Monitoring + Webhooks    │      Company-Centric       + weekly calibration
...
**Lead Score:** Calculated by `scoring/lead_score.py` (v1.5) using 5 weighted components: Intent (10%) + Engagement (10%) + Size (35%) + Seniority (30%) + Industry Fit (15%). Score range 0-100. 80% COLD is normal for cold Apollo data — this is expected, not a problem.

**Intent Proxy:** `enrichment/job_postings_enricher.py` uses Apollo Job Postings API to score company hiring intent (0-100). Companies actively hiring in relevant roles = higher buying intent.

**Auto Sequences:** `automation/auto_sequence.py` maps (Lead Tier, Role Category) → specific Apollo sequence. 24 sequences total across 2 tiers x 5 roles x sender variants.

**Self-Learning:** `scoring/score_calibrator.py` analyzes actual engagement outcomes and recommends weight adjustments. Safety rails prevent sudden jumps (max 10% change per cycle).

**Meeting Intelligence (Phase 3.5):** `meetings/meeting_tracker.py` syncs meetings, `meetings/meeting_analyzer.py` extracts AI intelligence (requires ANTHROPIC_API_KEY in GitHub Secrets), `meetings/opportunity_manager.py` converts positive meetings into pipeline opportunities with stage advancement and stale deal detection.

**Company-Centric Operating Model (v5.0):** All automation operates at the company level. ONE task per company per tier. Primary Company Owner derived from contact owner with most activity. Company Stage state machine: Prospect → Outreach → Engaged → Meeting → Opportunity → Customer → Churned → Archived.
...
| Phase 3.7 | **COMPANY-CENTRIC** — v5.0 Operating Model (Company Ownership, Company Metrics, Company Stage) | Complete (April 2026) |
| Phase 4 | **OPTIMIZE** — Odoo ERP + Revenue Tracking + Advanced Analytics | Planned |

Full details in `docs/EXECUTION_PLAN_v3.2.docx`

## Key Documents

| Document | Location | Purpose |
|----------|----------|---------|
| Deep-Dive Analysis Report | `docs/AI_Sales_OS_Deep_Analysis.md` | Full 12-section system analysis — strategy, tech, AI quality, gap analysis, risks, action plan |
| Meeting Intelligence Assessment | `docs/Meeting_Call_Intelligence_Architecture_Assessment.md` | Phase 3.5 architecture decision + rollout plan |
| Execution Plan | `docs/EXECUTION_PLAN_v3.2.docx` | Phase-by-phase detailed plan |
| Field Mapping Rules | `docs/System Architecture/FIELD_MAPPING_RULES.md` | Apollo → Notion field mapping |
| GitHub Ac