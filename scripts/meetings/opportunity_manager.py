...
from core.constants import (
    ...
    # Stage conflict guards
    STAGE_TERMINAL, is_stage_regression,
    # Task fields
...
def update_company_stage_to_opportunity(company_id: str, dry_run: bool = False) -> bool:
    ...
    Conflict Guard: reads current stage first, skips if terminal (Customer/
    Churned/Archived) or if the transition would be a regression.
    """
    # Read current stage before writing
    current_stage = ""
    try:
        resp = notion_request("GET", f"{NOTION_BASE_URL}/pages/{company_id}")
        ...
        logger.warning(f"[Conflict Guard] Could not read current stage for {company_id}: {e}")

    if current_stage in STAGE_TERMINAL:
        logger.info(
            f"[Conflict Guard] Skipping stage → Opportunity for {company_id}: "
            f"current '{current_stage}' is terminal"
        )
        return False

    if is_stage_regression(current_stage, COMPANY_STAGE_OPPORTUNITY):
        logger.info(
            f"[Conflict Guard] Prevented stage regression for {company_id}: "
            f"{current_stage} → {COMPANY_STAGE_OPPORTUNITY}"
        )
        return False

    if current_stage == COMPANY_STAGE_OPPORTUNITY:
        return True  # already at target, no-op

    if dry_run:
        logger.info(
            f"  [DRY RUN] Would update Company Stage → Opportunity for {company_id} "
            f"(was: {current_stage or 'empty'})"
        )
        return True
    try:
        ...
        logger.info(
            f"  Updated Company Stage → Opportunity for {company_id} "
            f"(was: {current_stage or 'empty'})"
        )
        return True
    except Exception as e:
