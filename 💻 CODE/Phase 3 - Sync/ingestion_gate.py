...
Usage:
    ...
    from ingestion_gate import CompanyGate, ContactGate
...
class CompanyGate:
    ...
    def _check_engagement(self, contacts: List[Dict]) -> GateCriterion:
        """Check if any contact at the company has engagement signals."""
        signals_found = []

        for c in contacts:
            for signal in ENGAGEMENT_SIGNALS:
                if c.get(signal):
                    signals_found.append(f"{c.get('name', '?')}: {signal}")

        return GateCriterion(
            name="Engagement Signal",
            passed=len(signals_found) > 0,
            detail=f"{len(signals_found)} signals" if signals_found else "No engagement",
        )
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true",
                        help="Run in audit mode regardless of --mode")
    ...
    mode = GOVERNANCE_MODE_AUDIT if args.dry_run else args.mode
    ...
    logger.info("      from ingestion_gate import IngestionGateRunner")
